...
# 3. One-time preprocessing (~10-15 minutes, creates tabix-indexed GTFs)
...
Before running the pipeline, ensure these files are accessible and paths are correctly set in `config.R` (see [Setup](#setup)):
...
2. Verify file paths in `config.R` (see [Setup](#setup))
